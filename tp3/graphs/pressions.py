...
def pressure(file):
    ...
    delta_ts = [i * delta_t for i in range(int(delta_t / delta_t), int(time / delta_t))]

    return delta_ts, counts, o_counts


delta_ts, counts, o_counts = pressure(parser.filename)
plt.plot(delta_ts, counts, label = 'Pression for walls')
plt.plot(delta_ts, o_counts, label = 'Pression for center particle')
